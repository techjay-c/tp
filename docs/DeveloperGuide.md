...
| `* * *`  | receptionist  | filter for patients using any attributes of a patient | filter patients with a keyword from an attribute                 |
| `* * *`  | receptionist  | filter for dentists using any attributes of a dentist | filter dentists with a keyword from an attribute                 |
| `* * *`  | receptionist  | create a new treatment                                | record what treatments are done in an appointment                |
| `* * *`  | receptionist  | list all treatments available                         | check what treatments are available in the clinic                |
...
| `* * *`  | receptionist  | filter for appointments using Patient ID              | view all appointments that patient has had in the clinic         |
| `* * *`  | receptionist  | filter for appointments using Dentist ID              | view all appointments that dentist has had in the clinic         |
| `* * *`  | receptionist  | view clinic schedule in a calendar                    | have an overview of the clinic's schedules to manage admin works |
...
(For all use cases below, the **System** is the `AddressBook` in ToothTracker and the **Actor** is the `user`, unless specified
otherwise)
